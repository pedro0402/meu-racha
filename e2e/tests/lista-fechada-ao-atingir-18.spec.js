...
test('ao atingir o limite configurado, a lista fecha em tempo real', async ({
  page,
  request,
}) => {
  ...
  await expect(page.getByText(/0 de 4 jogadores/)).toBeVisible();

  // Insere jogadores via API (mais rápido que pela UI)
  for (let i = 0; i < 4; i++) {
  ...
  await expect(page.getByText(/4 de 4 jogadores/)).toBeVisible();
